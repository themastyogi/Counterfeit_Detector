...
function calculateLogoSimilarity(scannedLogos, referenceLogos, brandName, scannedText) {
    ...
    if (brandName && scannedText) {
        ...
        console.log(`🔍 Checking text fallback: Brand="${brandName}" in Text? ${textLower.includes(brandLower)}`);
        ...
        console.log(`⚠️ Text fallback skipped: Brand="${brandName}", TextLength=${scannedText?.length}`);
    }
